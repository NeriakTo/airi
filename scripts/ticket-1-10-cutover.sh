...
SKILLS_MOVED=0            # step2 成功搬離後＝1（補償時需搬回；重排後 skills 搬離為 step2）
...
# ── plugin list 狀態解析（R6 真因修復；R6b 依黑喵審查強化 fail-closed／區塊邊界）──────
#   真實 `claude plugin list` 為「多行區塊」格式：「  ❯ <id>」標題行（marker＝❯），其下數行縮排
#   欄位，狀態獨立在「    Status: <glyph> <state>」行；區塊之間以空行分隔。
#   舊碼誤將整份輸出 grep 出含 <id> 的行（＝標題行）再於「該行」找 disabl/enabl——狀態字不在標題行，
#   真實輸出永遠 miss（R6 假綠根因：mock 把 id 與 status 印成同一行，故 grep 命中）。
#   R6b 強化（黑喵審）：
#     1) 區塊關閉不可只靠空行——遇「任一 plugin 標題」即關；否則 CLI 若省略區塊間空行，會把 sibling 的
#        Status 誤讀成目標狀態（fail-open）。故標題以 marker `$1=="❯"` 辨識，僅「marker＋末欄==id」才開區塊，
#        其餘標題（含撞名說明行 `❯ meowvoice@skills-dir: ...`，其末欄非 id）一律關。
#     2) 回傳「正規化狀態 token」（Status 行末欄轉小寫，如 disabled/enabled/loaded），caller 以「精確相等」
#        判定，杜絕 substring 假放行（如「failed to disable」含 disable、「not enabled」含 enabled）。查無＝空＝fail-closed。
plugin_present(){   # $1=id；$2=plugin list 輸出；有＝印 "yes"，無＝空
  printf '%s\n' "$2" | awk -v id="$1" '$1=="❯" && $NF==id { print "yes"; exit }'
}
plugin_block_status(){   # $1=id；$2=plugin list 輸出；stdout＝該區塊狀態 token（小寫；查無/格式非預期＝空）
  printf '%s\n' "$2" | awk -v id="$1" '
    NF==0     { inblk=0; next }                       # 空行＝區塊分隔（保險，非唯一邊界）
    $1=="❯"   { inblk=($NF==id)?1:0; next }            # 任一 plugin 標題：僅目標(marker＋末欄==id)開，其餘關
    inblk && index($0,"Status:")>0 {                  # 目標區塊內首個 Status 行→驗格式後取狀態 token
      sub(/.*Status:[[:space:]]*/,"")                 # 去到 "<glyph> <state>"（正常 2 欄）或 "<state>"（無 glyph，1 欄）
      # fail-closed：正常狀態格式至多 2 欄（glyph＋單詞 state）；>2 欄＝非預期（如「not enabled」「failed to disable」）→不輸出。
      if (NF>=1 && NF<=2) print tolower($NF)
      exit
    }'
}
...
do_early_compensation(){   # launcher 未換裝（step1/2/3）→ 分步補償，不做 launcher 檔案還原
  ...
  # 1. best-effort 清除 marketplace（重排後 step3 可能已 add/install）
  ...
  # 2. 若 skills 已搬離（重排後 step2 先搬；step3 marketplace 失敗時 SKILLS_MOVED 仍為 1），把本次 archive 搬回。
  #    R6b（黑喵審）：skills 還原是回滾「完整性」的一環，不可只記 WARN 就放行——若還原失敗（mv 失敗、
  #    備份不存在、或還原後仍缺 server.ts），系統處於「舊 launcher 要載 skills-dir 但 skills 已不在」的
  #    不完整態，必須強制 FAILED_NEED_MANUAL，不得僅憑語音鏈三合一健康就誆稱 FAILED_ROLLED_BACK。
  local restore_ok=1   # 本次未搬 skills（SKILLS_MOVED=0）＝無需還原，視為 ok
  if [ "$SKILLS_MOVED" -eq 1 ]; then
    restore_ok=0
    local sb="${SKILLS_ARCHIVE}/meowvoice-pre-1-10-${STAMP}"
    # R6c（黑喵三審）：只有「本次 mv 明確成功 且 搬回後 server.ts 存在」才算還原完成（restore_ok=1）。
    # 既存目標（不覆蓋）、備份不存在、mv 失敗一律保持 0——不得因「另一份既存 server.ts」而誤判還原成功
    # （fail-open：本次 archive 其實沒搬回卻假報 FAILED_ROLLED_BACK）。server.ts 檢查收進 mv 成功分支內。
    if [ -e "$SKILLS_LIVE" ]; then
      log "[RECOVER][WARN] 搬回前 ${SKILLS_LIVE} 已存在，未覆蓋（本次備份未還原）——需人工確認"
    elif [ ! -d "$sb" ]; then
      log "[RECOVER][WARN] 找不到本次 skills 備份 ${sb}——無法還原"
    elif mv "$sb" "$SKILLS_LIVE" 2>/dev/null && [ -f "${SKILLS_LIVE}/server.ts" ]; then
      restore_ok=1; log "[RECOVER] skills 還原確認（本次備份已搬回且 server.ts 存在）"
    else
      log "[RECOVER][WARN] skills 搬回 mv 失敗或還原後缺 server.ts——回滾不完整"
    fi
  fi
  ...
  # 4. 終態：skills 還原（若有搬）＋語音鏈三合一都成立才算 FAILED_ROLLED_BACK；任一未達＝FAILED_NEED_MANUAL。
  if [ "$restore_ok" -ne 1 ]; then
    write_terminal_marker "FAILED_NEED_MANUAL" "${ctx}（skills 搬回失敗／還原後缺 server.ts——回滾不完整，需人工還原 skills 後再驗語音鏈）"
  elif verify_voice_chain_restored; then
    write_terminal_marker "FAILED_ROLLED_BACK" "${ctx}（分步補償完成：skills 還原＋語音鏈三合一皆獨立驗證通過）"
  else
...
print_action_list(){
  ...
  步驟 2 舊 skills 源目錄搬離【監督 STOPPED】（重排：先搬，消撞名）
    - mv ~/.claude/skills/meowvoice ~/.claude/skills-archive/meowvoice-pre-1-10-<STAMP> ; 驗不存在
  步驟 3 marketplace 註冊/安裝/disable【監督 STOPPED】（skills 已搬，安裝無撞名）
    - claude plugin marketplace add ~/github/meowvoice/plugin-marketplace
    - claude plugin install / disable meowvoice@meowvoice-local ; 驗該 id 區塊 Status 行 ✘ disabled
    - 3c 預熱：bun install --no-summary（installPath）→ 驗 @modelcontextprotocol/sdk
...
step3_marketplace(){
  # 重排（Kevin 選 1）：skills 已於步驟 2 先搬離，此處安裝 marketplace 版時 skills-dir 已不在，
  # 不再產生「name already taken／takes precedence」撞名行，plugin list 該 id 區塊唯一、乾淨。
  log "== 步驟 3：註冊 marketplace、安裝、disable（skills 已於步驟 2 搬離，無撞名）=="
  ...
  # R6 真因修復：狀態在該 id 區塊的獨立 Status 行，不在標題行——用 plugin_block_status 抽 Status 行判定。
  if [ -z "$(plugin_present 'meowvoice@meowvoice-local' "$pl")" ]; then log "  [ERR] plugin list 未見 meowvoice@meowvoice-local"; return 1; fi
  st="$(plugin_block_status 'meowvoice@meowvoice-local' "$pl")"
  if [ "$st" != "disabled" ]; then log "  [ERR] meowvoice 未呈 disabled（狀態 token='${st}'，精確比對 fail-closed）"; return 1; fi
  log "  list 顯示 disabled（狀態 token='${st}'）"
  local install_path
...
step2_move_skills(){
  # 重排（Kevin 選 1）：搬離提前到 marketplace 安裝之前，確保安裝時無同名 skills-dir 副本＝無撞名。
  log "== 步驟 2：舊 skills 源目錄搬離（防雙載；先於 marketplace 安裝，消撞名）=="
...
step45_enable(){
  ...
  log "  --- claude plugin list ---"; log "$pl"
  if [ "$plrc" -ne 0 ]; then log "  [ERR] plugin list 查詢失敗 rc=${plrc}（fail-closed）"; return 1; fi
  # R6 真因修復：讀該 id 區塊 Status 行判定 enabled（非標題行）。
  if [ -z "$(plugin_present 'meowvoice@meowvoice-local' "$pl")" ]; then log "  [ERR] plugin list 未見 meowvoice"; return 1; fi
  st="$(plugin_block_status 'meowvoice@meowvoice-local' "$pl")"
  if [ "$st" = "enabled" ]; then log "  list 顯示 enabled（狀態 token='${st}'）"; return 0; fi
  log "  [ERR] meowvoice 未呈 enabled（狀態 token='${st}'，精確比對 fail-closed）"; return 1
}
...
step6_verify(){
  ...
  if [ "$plrc" -ne 0 ]; then log "  6d [FAIL] plugin list 查詢失敗 rc=${plrc}"; ok=1; else
    # R6 真因修復：讀該 id 區塊 Status 行判定 enabled（非標題行）。
    st="$(plugin_block_status 'meowvoice@meowvoice-local' "$pl")"
    if [ "$st" = "enabled" ]; then log "  6d enabled（狀態 token='${st}'）"; else log "  6d [FAIL] 未 enabled（狀態 token='${st}'，精確比對 fail-closed）"; ok=1; fi
  fi
...
finalize_success(){
  ...
  if ! pl="$("$CLAUDE_BIN" plugin list 2>&1)"; then
    ...
    # R6 真因修復：讀該 id 區塊 Status 行判定 enabled（非標題行）。
    if [ -z "$(plugin_present 'meowvoice@meowvoice-local' "$pl")" ]; then echo "  F3 plugin 未見 meowvoice@meowvoice-local——拒升"; fail=1
    else
      st="$(plugin_block_status 'meowvoice@meowvoice-local' "$pl")"
      if [ "$st" = "enabled" ]; then echo "  F3 plugin marketplace 版 enabled: yes（狀態 token='${st}'）"
      else echo "  F3 plugin 非 enabled（狀態 token='${st}'）——拒升"; fail=1; fi
    fi
  fi
...
real_run(){
  ...
  # 步驟 2/3 重排（Kevin 選 1）：先搬 skills 再裝 marketplace，安裝當下無同名 skills-dir＝無撞名。
  # 兩步同屬「launcher 未換裝」的早期補償桶（do_early_compensation 同時清 marketplace＋搬回 skills），
  # 故互換不影響補償正確性（見 do_early_compensation）。
  CUR_STEP="step1"; step1_shutdown    || recover_and_mark "步驟 1 受控停機"
  CUR_STEP="step2"; step2_move_skills || recover_and_mark "步驟 2 舊 skills 源目錄搬離"
  CUR_STEP="step3"; step3_marketplace || recover_and_mark "步驟 3 marketplace 註冊/安裝/disable"
