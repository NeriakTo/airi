...
## 2. 註冊 marketplace、安裝、disable【監督：STOPPED（install→disable 窗口無害）】
...
claude plugin list                                       # 期望：meowvoice@meowvoice-local  Status: ✘ disabled

# 2c. 預熱依賴（避免首次載入時 `bun install` 受網路影響延遲語音就緒）
...
驗證：add/install/disable exit 0；list 顯示 **✘ disabled**；2c 印 `deps OK`。

---

## 3. 舊 skills 源目錄搬離（防雙載）【監督：STOPPED】
...
claude plugin list        # 期望：meowvoice@meowvoice-local  Status: ✔ enabled
```

驗證：list 顯示 **✔ enabled**。（skills-dir 版已搬離、launcher 已換、監督仍停，enable 後無雙載風險。）
...
# 6d. plugin 已載入
claude plugin list        # 期望：meowvoice@meowvoice-local ✔ enabled
